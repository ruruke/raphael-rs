...
impl<'alloc> QualityUbSolver<'alloc> {
    ...
    fn solve_precompute_state(
        ...
        for cp in (0..=(state.cp - 2)).step_by(2).rev() {
            let lesser_state = ReducedState { cp, ..state };
...
impl<'main, 'alloc> QualityUbSolverShard<'main, 'alloc> {
    ...
    fn solve_state(
        ...
        for cp in (0..=(state.cp - 2)).step_by(2).rev() {
            let lesser_state = ReducedState { cp, ..state };
            let shared_solution = self.shared_states.get(&lesser_state).copied();
            let local_solution = self.local_states.get(&lesser_state).copied();
            if let Some(pareto_front) = shared_solution.or(local_solution) {
                let lesser_state_is_maximal = pareto_front.first().progress >= cutoff.progress
                    && pareto_front.first().quality >= cutoff.quality;
                if lesser_state_is_maximal {
